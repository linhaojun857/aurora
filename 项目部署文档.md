...
>
> 下面的命令分别对mysql的日志文件、数据文件进行了映射，你也可以自己修改。

```shell
docker run -d -p 3306:3306 --restart=always -v /auroras/mysql/log:/var/log/mysql -v /auroras/mysql/data:/var/lib/mysql -e MYSQL_ROOT_PASSWORD=密码  --name mysql mysql
```
...
> 如果遇到出现java:8镜像拉取失败的，可以自己换一个镜像即可解决问题。
