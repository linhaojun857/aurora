...
本项目支持 Apollo 配置，前提是需要自己部署 Apollo 配置服务，Apollo 的部署可以参考这个文档：[Apollo 分布式部署指南](https://www.apolloconfig.com/#/zh/deployment/distributed-deployment-guide)。

如果你想集成 Apollo，并且已经部署了 Apollo 配置服务，那么需要做如下操作：
...
2. 增加启动参数：`-Dapollo.meta=http://config-service-url:port -Dapp.id=YOUR-APP-ID -Denv=YOUR-ENVIRONMENT`。这几个参数的值同样需要根据情况而定。

如果出现问题，请参考：

- [跳过Apollo Meta Server服务发现](https://www.apolloconfig.com/#/zh/usage/java-sdk-user-guide?id=_1222-跳过apollo-meta-server服务发现)
- [部署&开发遇到的常见问题](https://www.apolloconfig.com/#/zh/faq/common-issues-in-deployment-and-development-phase)



如果你不想集成 Apollo 配置，需要做如下操作：

1. 删除`AuroraSpringbootApplication`中的注解：`@EnableApolloConfig`
